...
class Игра(object):
    ...
    def Управление(self,Действие,Начало,Окно,Видить,Время):
        ...
        #if Действие==1 and self.Скорость > МаксСкНаз:
...
while True:
    ...
    if not Закрыли:
        ...
        if a == 6:
            ...
            сменаEps = 1
    пп = len(Буфер2)
    if len(Буфер2)!=0 and not Перехват:
    ...
    if Эпизод % печать_интервалов == 0 and Эпизод!=0:
        ...
        print("# Эпизод: {}, Награда: {:.1f}, размер буфера: {}, Eps: {:.2f}%, Рекорд: {:.2f}".format(
             Эпизод,ОбщаяНаграда/печать_интервалов,100,Eps*100,Рекорд))
        ОбщаяНаграда = 0.0
        Рекорд = 0
