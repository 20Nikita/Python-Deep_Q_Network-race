...
class Игра(object):
    ...
    def Лучи (self):
        self.Среда = [[10000,10000],[10000,10000],[10000,10000],[10000,10000],[10000,10000],[10000,10000],[10000,10000],[10000,10000],[10000,10000],[10000,10000],[10000,10000],[10000,10000],[10000,10000],[10000,10000],[10000,10000],[10000,10000]]
        цeнтр = (self.pos[0]+self.ш/2,self.pos[1]+self.в/2)
        for j in range(self.Лучей):
            for i in range(len(self.Трасса)-1):
                t = self.intersection(self.Трасса[i], self.Трасса[i+1], (цeнтр[0],цeнтр[1]),
                                      (цeнтр[0] - self.Длинна * math.sin(math.radians(self.Угол + j*360/self.Лучей)),
                                      цeнтр[1] - self.Длинна * math.cos(math.radians(self.Угол + j*360/self.Лучей))))
    ...
    def Управление(self,Действие,Начало,Окно,Видить,Время,Долго):
        МаксСкВп = 6
        МаксСкНаз = -2
        Ускорение = 0.5
        Трение = 0
    ...
    def Графика(self, Окно,Время,Долго, Видно_Дорогу = True, Видно_Лилии = True, Видно_Круги = True, Видно_Дданные = True):
        ЦветФона = (50, 50, 50)
        ...
        if Видно_Лилии:
            for i in range(self.Лучей):
                pygame.draw.aaline(Окно, ЦветЛиний, (цeнтр[0],цeнтр[1]), (цeнтр[0] - self.Длинна * math.sin(math.radians(self.Угол + i*360/self.Лучей)),цeнтр[1] - self.Длинна*math.cos(math.radians(self.Угол + i*360/self.Лучей))))
        if Видно_Круги:
            for i in range(len(self.Среда)):
        ...
        Текст = Шрифт.render(f"Пройденный путь: {self.Растояние:.2f} Время: {Время.get_time()} Попыток: {self.Попыток} Время: {int(Долго/1000)}", 0, ЦветЛиний)
...
class QNetwork(nn.Module):
    def __init__(self):
        ...
        self.сл1 = nn.Linear(16,256)        
        self.сл10 = nn.Linear(256,2)
...
while True:
    ...
    #Постепенно изменяем Eps с 8% до 1%
    Eps = max(0.001, 0.08 - 0.00011*(Эпизод))
    ...
    if not Закрыли:
        ...
        while Играем:
            ...
            #Выполняем действие в окружении
            НовоеОкружение, Награда, Закончил = игра.Управление(Действие,Начало,Окно,Видить,Время,Долго)
           ...
           #     игра.Обновить()
            ...
            #if Награда > 0:
            #    for i in range(10):
            #        Буфер.put((Окружение,Действие,Награда, НовоеОкружение))
